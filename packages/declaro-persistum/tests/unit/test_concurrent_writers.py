...
all, stays synced — and it does not need MVCC to make its point.
...
async def _pool(tmp_path, monkeypatch, *, synced: bool):
    """`synced` is required. It is the ONLY thing that decides the engine mode.
    ...
    assert pool._mvcc is (not synced), (
