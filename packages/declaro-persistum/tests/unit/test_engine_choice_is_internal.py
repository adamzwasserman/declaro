...
call site. Worse, `mvcc` defaulted to True, which on a SYNCED pool is the
...
def test_a_synced_pool_never_requests_mvcc(tmp_path):
    """MVCC on a synced replica is the stranding defect. It must be impossible."""
...
def test_the_choice_follows_remote_url_and_nothing_else(tmp_path):
    ...
    synced = TursoPool(str(tmp_path / "a"), remote_url="https://x.turso.io")
    local = TursoPool(str(tmp_path / "b"))
    assert (synced._mvcc_requested, local._mvcc_requested) == (False, True)
