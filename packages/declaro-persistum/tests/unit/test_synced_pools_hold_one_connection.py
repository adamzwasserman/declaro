"""A synced pool holds ONE write connection. A local pool opens one per write.
...
unconditionally — no branch on whether the pool was synced.

Every pyturso connection is its own OS worker thread, and on a SYNCED pool
...
(declaro-eer): a synced replica takes ONE sync connection, and opening a
...
wrong on a synced pool, which 0.1.28 knew and I discarded.

The vocabulary is two members, synced and local, so both run here rather than
...
async def _pool(tmp_path, monkeypatch, *, synced: bool):
    ...
    remote = (
        {"remote_url": "https://example.turso.io", "auth_token": "t"} if synced else {}
    )
...
@pytest.mark.asyncio
async def test_a_synced_pool_opens_no_connection_per_write(tmp_path, monkeypatch):
    """The regression, stated as the count that changed.

    Twenty writes must not cost twenty sync-engine handshakes against the
    replica tape.
    """
    pool = await _pool(tmp_path, monkeypatch, synced=True)
    ...
    assert len(_Holder.opened) == 0, (
        f"{len(_Holder.opened)} new sync connections for {WRITES} writes on a "
        f"SYNCED pool; a synced replica takes one (declaro-eer), and opening "
        f"per write is what killed multicardz's stage box (declaro-dna)"
    )


@pytest.mark.asyncio
async def test_a_synced_pool_reuses_the_held_connection(tmp_path, monkeypatch):
...
@pytest.mark.asyncio
async def test_a_synced_pool_does_not_close_its_connection_after_a_write(
    ...
    assert held.conn is not None, "the synced pool closed its write connection"
...
@pytest.mark.asyncio
async def test_concurrent_writers_on_a_synced_pool_open_nothing(
    tmp_path, monkeypatch
):
