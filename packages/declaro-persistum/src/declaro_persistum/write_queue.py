...
class WriteQueue:
    ...
    def __init__(
        ...
        # Keys whose original write is still running. The supervisor must not
        # retry these: the write that put them here has not finished, and
        # applying it twice inserts the row twice or fails the second attempt
        # on the primary key.
    ...
    def mark_in_flight(self, table: str, pk_value: Any) -> None:
        """Claim an entry whose original write is still running.

        The supervisor skips a claimed entry. Release it with
        clear_in_flight when that write fails, or let remove_entry drop it
        when the write succeeds.
        """
        self._in_flight.add(f"{table}:{pk_value}")

    def clear_in_flight(self, table: str, pk_value: Any) -> None:
        """Release an entry so the supervisor takes it over."""
        self._in_flight.discard(f"{table}:{pk_value}")

    @property
    def threshold_seconds(self) -> float:
        """How long a caller waits for a write before it is queued instead."""
        return self._threshold_ms / 1000.0

    def note_failed_attempt(self, table: str, pk_value: Any, exc: Exception) -> None:
        """Record that the in-flight write for a queued entry failed.

        The entry stays queued and the supervisor keeps retrying it. This
        only records what went wrong, so a row that never lands is
        diagnosable rather than silently stuck.
        """
        key = f"{table}:{pk_value}"
        ...
        while True:
            ...
            for key, entry in list(self._queue.items()):
                if key in self._in_flight:
                    continue  # its original write is still running
