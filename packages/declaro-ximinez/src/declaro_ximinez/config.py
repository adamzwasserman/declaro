"""Configuration loading for declaro-ximinez."""
...
from .types import XiminezConfig


DEFAULT_CONFIG: XiminezConfig = {
    "enabled": True,
    "stage": 1,
...
def load_config(project_root: Path | None = None) -> XiminezConfig:
    """Load ximinez configuration from pyproject.toml.
...
def load_config_from_file(pyproject_path: Path) -> XiminezConfig:
    """Load ximinez configuration from a specific pyproject.toml.
    ...
    tool_config = data.get("tool", {}).get("ximinez", {})

    # Merge tool.ximinez settings
    ...
    # Merge tool.ximinez.declaro settings
...
def parse_file_directive(source: str) -> dict[str, str]:
    """Parse ximinez directives from file comments.
    ...
    for line in source.split("\n"):
        line = line.strip()
        if not line.startswith("# ximinez:"):
            continue

        # Parse the directive
        directive_str = line[10:].strip()  # Remove "# ximinez:"
